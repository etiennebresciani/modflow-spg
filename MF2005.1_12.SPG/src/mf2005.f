...
C-------ASSIGN VERSION NUMBER AND DATE
      ...
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='1.12.SPG 7/16/2021 (Etienne Bresciani)')
...
c      IF(IUNIT(14).GT.0) CALL LMG7AR(IUNIT(14),MXITER,IGRID)
     ...
     2                     IUNIT(23),IUNIT(37),IUNIT(62),IGRID)
      IF(IUNIT(66).GT.0) CALL GWF2SPG7AR(IUNIT(66),IGRID)               SPG7
...
C----------READ USING PACKAGE READ AND PREPARE MODULES.
     ...
     1                            IUNIT(23),IUNIT(37),KKPER,IGRID)
        ...
        IF(IUNIT(66).GT.0) CALL GWF2SPG7RP(IUNIT(66),IGRID)             SPG7
C
...
C-----------CALL GWF2SPG7FM BEFORE CALCULATING CONDUCTANCES
C-----------BECAUSE IT NEEDS THOSE OF THE PREVIOUS ITERATION.
            IF(IUNIT(66).GT.0) CALL GWF2SPG7FM(KKPER,IGRID)             SPG7
...
C7C4----CALCULATE BUDGET TERMS. SAVE CELL-BY-CELL FLOW TERMS.
          MSUM = 1
C---------CALL GWF2SPG7BD FIRST BECAUSE IT SWITCHES BACK TEMPORARY
C---------CONSTANT HEAD CELL TO INTERNAL CELL.
          IF(IUNIT(66).GT.0) CALL GWF2SPG7BD(KKSTP,KKPER,IGRID)         SPG7
          IF (IUNIT(1).GT.0) THEN
...
c      IF(IUNIT(14).GT.0) CALL LMG7DA(IGRID)
     ...
     1                                              IGRID)
      ...
      IF(IUNIT(66).GT.0) CALL GWF2SPG7DA(IGRID)
